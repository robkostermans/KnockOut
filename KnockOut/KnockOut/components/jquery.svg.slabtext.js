    ...
    $.fn.slabText = function (options) {
        ...
        return this.each(function () {
            ...
            var resizeSlabs = function resizeSlabs() {
                ...
                $("span.slabtext", $this).each(function ( index ) {
                    var $span = $(this),
                    ...
                    $(SVG('text'))
                        .attr('x', 0)
                        .attr("y", Math.min((fontSize * ratio).toFixed(precision) / 12, settings.maxFontSize / 12) - 1 + "px")
                        //.attr('dy', '1')
                        .attr((wordSpacing ? 'word' : 'letter') + '-spacing', (diff / (wordSpacing ? innerText.split(" ").length - 1 : innerText.length)).toFixed(precision) + "px")
                        .attr("font-size", Math.min((fontSize * ratio).toFixed(precision) / 12, settings.maxFontSize / 12) - 1 + "px")
                        .text(innerText)
                        .appendTo($("#mask"));
                });
